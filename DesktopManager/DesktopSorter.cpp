...
#include "ItemTypes.h"                   // For ItemCategory
#include <algorithm>
#include <cctype>
#include <memory>
#include <string>
...
void DesktopSorter::setItemPosition(const std::wstring& itemName, int x, int y) { /* ... implementation with logging ... */
    ...
    if (itemIndex != -1) {
        ...
        else { LOG_INFO(L"DesktopSorter::setItemPosition - Successfully moved item '" + itemName + L"'."); }
    } else { LOG_WARNING(L"DesktopSorter::setItemPosition - Item '" + itemName + L"' not found in ListView."); }
#else
    LOG_DEBUG(L"DesktopSorter::setItemPosition (stub) - Item: " + itemName + L", Pos: (" + std::to_wstring(x) + L"," + std::to_wstring(y) + L")");
#endif
}

ItemCategory DesktopSorter::classifyItem(DesktopItem& item) { // Changed to non-const item&
    LOG_DEBUG(L"DesktopSorter::classifyItem - Attempting to classify item: " + item.name);
    ItemCategory determinedCategory = ItemCategory::UNCLASSIFIED;
    ...
    if (item.type == DesktopItem::ItemType::FOLDER) {
        ...
        determinedCategory = ItemCategory::FOLDER;
    } else {
        ...
        if (extension == L"txt" || extension == L"log" || extension == L"md" || extension == L"doc" || extension == L"docx" || extension == L"pdf") {
            ...
            determinedCategory = ItemCategory::TEXT_FILE;
        } else if (extension == L"png" || extension == L"jpg" || extension == L"jpeg" || extension == L"gif" || extension == L"bmp" || extension == L"svg") {
            LOG_INFO(L"DesktopSorter::classifyItem - Item '" + item.name + L"' classified as IMAGE_FILE by extension.");
            determinedCategory = ItemCategory::IMAGE_FILE;
        } else if (extension == L"mp4" || extension == L"mkv" || extension == L"avi" || extension == L"mov" || extension == L"wmv") {
            LOG_INFO(L"DesktopSorter::classifyItem - Item '" + item.name + L"' classified as VIDEO_FILE by extension.");
            determinedCategory = ItemCategory::VIDEO_FILE;
        } else if (extension == L"zip" || extension == L"rar" || extension == L"7z" || extension == L"tar" || extension == L"gz") {
            LOG_INFO(L"DesktopSorter::classifyItem - Item '" + item.name + L"' classified as ARCHIVE_FILE by extension.");
            determinedCategory = ItemCategory::ARCHIVE_FILE;
        } else {
#ifdef _WIN32
            if (m_webClassifier && (item.type == DesktopItem::ItemType::SHORTCUT ||
                ...
                if (webCategory == ItemCategory::GAME || webCategory == ItemCategory::PROGRAM) {
                    LOG_INFO(L"DesktopSorter::classifyItem - Item '" + item.name + L"' classified as " + itemCategoryToWString_ds(webCategory) + L" by WebClassifier.");
                    determinedCategory = webCategory;
                } else {
                    LOG_INFO(L"DesktopSorter::classifyItem - WebClassifier returned UNCLASSIFIED for: " + item.name + L". Falling back to basic rules.");
                }
            }
#endif
            if (determinedCategory == ItemCategory::UNCLASSIFIED &&
                (item.type == DesktopItem::ItemType::SHORTCUT || extension == L"exe" || extension == L"app" || extension == L"bat" || extension == L"sh")) {
                if (lowerName.find(L"game") != std::wstring::npos || lowerName.find(L"игра") != std::wstring::npos ||
                    lowerName.find(L"play") != std::wstring::npos || lowerName.find(L"steam") != std::wstring::npos) {
                    LOG_INFO(L"DesktopSorter::classifyItem - Item '" + item.name + L"' classified as GAME by basic name rules.");
                    determinedCategory = ItemCategory::GAME;
                } else if (lowerName.find(L"editor") != std::wstring::npos || lowerName.find(L"browser") != std::wstring::npos ||
                           lowerName.find(L"office") != std::wstring::npos || lowerName.find(L"player") != std::wstring::npos ||
                           lowerName.find(L"manager") != std::wstring::npos || lowerName.find(L"studio") != std::wstring::npos ||
                    ...
                    determinedCategory = ItemCategory::PROGRAM;
                } else if (extension == L"exe" || item.type == DesktopItem::ItemType::SHORTCUT) {
    ...
    if (determinedCategory == ItemCategory::UNCLASSIFIED &&
        (toLower_wstring_internal_ds(item.name) == L"recycle bin" || toLower_wstring_internal_ds(item.name) == L"корзина" ||
         toLower_wstring_internal_ds(item.name) == L"this pc" || toLower_wstring_internal_ds(item.name) == L"мой компьютер" ||
         toLower_wstring_internal_ds(item.name) == L"computer" ||
         toLower_wstring_internal_ds(item.name) == L"network" || toLower_wstring_internal_ds(item.name) == L"сеть")) {
        LOG_INFO(L"DesktopSorter::classifyItem - Item '" + item.name + L"' classified as SPECIAL_SYSTEM by name.");
        determinedCategory = ItemCategory::SPECIAL_SYSTEM;
    }
    ...
    item.category = determinedCategory; // Set the category on the item
